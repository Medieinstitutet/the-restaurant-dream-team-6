...
const BookingForm: React.FC = () => {
  ...
  const handleStep2Submit = (customerData: {
    name: string;
    lastname: string;
    email: string;
    phone: string;
    ...
    if (
      ...
      setErrorMessage(
        "Samtliga fält är obligatoriska och måste vara korrekt ifyllda."
      );
      return;
    }
    ...
    if (step === 3) {
      const makePostRequest = async () => {
        ...
        try {
          ...
          setErrorMessage(
            "Något gick fel vid bokning. Försök igen senare eller ring oss för att få hjälp."
          );
        } finally {
          setLoading(false);
        }
